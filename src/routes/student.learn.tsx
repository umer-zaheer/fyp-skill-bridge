    ...
    <div className="space-y-6" style={{ fontFamily: "Inter, sans-serif" }}>
      ...
      >
        ...
        <div className="relative flex flex-col md:flex-row gap-6 items-start md:items-center">
          <div className="relative h-40 w-full md:w-64 rounded-xl overflow-hidden border border-amber-500/20 bg-zinc-900 shrink-0">
            ...
            <div className="absolute inset-0 bg-zinc-950/40 flex items-center justify-center">
              <PlayCircle className="h-14 w-14 text-amber-400 drop-shadow-lg" />
            </div>
          </div>
          <div className="flex-1">
            <p className="text-xs uppercase tracking-[0.2em] text-amber-500/80">{current.course}</p>
            <h3 className="text-2xl font-semibold text-zinc-900 dark:text-white mt-2">{current.title}</h3>
            ...
            <Link
              ...
              params={{ id: "1" }}
              className="mt-5 inline-flex items-center gap-2 rounded-lg bg-amber-500 hover:bg-amber-400 px-5 py-2.5 text-sm font-semibold text-zinc-950 transition-colors"
            >
          ...
          <ul className="space-y-3">
            {upNext.map((l, i) => (
              <motion.li
                ...
                className="flex items-center gap-3 p-3 rounded-lg hover:bg-zinc-100 dark:hover:bg-white/5 border border-transparent hover:border-amber-500/20 transition-colors group cursor-pointer"
              >
